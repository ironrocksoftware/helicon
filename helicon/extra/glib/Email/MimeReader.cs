...
namespace glib.Email
{
    ...
    public class MimeReader
    {
        ...
        public RxMailMessage GetEmail(Stream EmailStream)
        {
            ...
            if (MessageMimeReturnCode == MimeEntityReturnCode.bodyComplete || MessageMimeReturnCode == MimeEntityReturnCode.parentBoundaryEndFound)
            {
                if (Root.To.Count == 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(sTo)) Root.To.Add(sTo);
                }

                if (Root.From == null)
                {
                    string sFrom = Root.Headers["x-sender"];
                    if (!string.IsNullOrEmpty(sFrom)) Root.From = new MailAddress(sFrom);
                }
        ...
        /// </summary>
        private MimeEntityReturnCode ProcessMimeEntity(RxMailMessage root, RxMailMessage message, string parentBoundaryStart)
        {
            ...
            while (readMultiLine(out response))
            {
                ...
                if (isBoundaryDefined && response.TrimEnd() == BoundaryDelimiterLineStart)
                {
                    ...
                    return ProcessDelimitedBody(root, message, BoundaryDelimiterLineStart, parentBoundaryStart, parentBoundaryEnd);
                }
        ...
        private MimeEntityReturnCode ProcessDelimitedBody(RxMailMessage root, RxMailMessage message, string BoundaryStart, string parentBoundaryStart, string parentBoundaryEnd)
        {
            string response;
            ...
            do
            {
                ...
                ReturnCode = ProcessMimeEntity(root, ChildPart, BoundaryStart);
                ...
                AddChildPartsToParent(root, ChildPart, message);
        ...
        static AlternateView DATA_TEXT;

        /// <summary>
        /// Add all attachments and alternative views from child to the parent
        /// </summary>
        private void AddChildPartsToParent (RxMailMessage root, RxMailMessage child, RxMailMessage parent)
        {
            ...
            if (child.ContentStream != null && child.Parent.ContentType != null)
            {
            	if (child.ContentStream.Length != 0)
            	{
...
//Console.WriteLine(">> "+child.ContentType.MediaType.ToLowerInvariant());
...
root.AlternateViews.Add(thisAlternateView);

switch(child.ContentType.MediaType.ToLowerInvariant())
{
	case "text/plain":
		MimeReader.DATA_TEXT = thisAlternateView;
		break;

	case "text/html":
		if (MimeReader.DATA_TEXT != null) root.AlternateViews.Remove(MimeReader.DATA_TEXT);
		MimeReader.DATA_TEXT = null;
		break;
}
            ...
            if (child.Attachments != null)
            {
                ...
                {
                	if (childAttachment.ContentStream.Length != 0)
